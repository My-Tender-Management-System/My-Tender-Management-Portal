        ...
        <ng-container>
            ...
            <a [attr.href]="item.url" (click)="itemClick($event)" *ngIf="(!item.routerLink || item.items) && item.visible !== false"
               ...
               [attr.target]="item.target" [attr.tabindex]="0" [ngClass]="item.class" pRipple>
                <i class="layout-submenu-toggler" *ngIf="item.items"></i>
            ...
            <a (click)="itemClick($event)" (mouseenter)="hover=true" (mouseleave)="hover=false"
               *ngIf="(item.routerLink && !item.items) && item.visible !== false"
               ...
               [ngClass]="item.class" pRipple [ngClass]="{'active-menuitem-routerlink': this.active}">
                <!-- <i [ngClass]="item.icon" class="layout-menuitem-icon"></i> -->
                <span [ngClass]="item.icon" *ngIf="item.materialIcon" class="material-symbols-outlined layout-menuitem-icon">{{ item.materialIcon }}</span>
                <span class="layout-menuitem-text">{{item.label}}</span>
                <i class="pi pi-fw pi-angle-down layout-submenu-toggler" *ngIf="item.items"></i>
                <span class="menuitem-badge" *ngIf="item.badge">{{item.badge}}</span>
            </a>
            <ul *ngIf="root && item.visible !== false">
